...
local FlackJC = ini.settings.key
	...
	if not isSampLoaded() or not isSampfuncsLoaded() then return end
  	...
  	--main_windows_state.v = not main_windows_state.v
		...
		if recon == true then
			...
			checkpass = false
			account_info = nil
			if tape1 == true then wait(100) reconifkick.v = true tape1 = false end
			for i, v in ipairs(accounts) do
		...
		--and (f_scrText_state or sat_flag)
		if (altbot.v or animsbot.v or chipsbot.v or fishbot.v) and (f_scrText_state or sat_flag) then
  ...
  if imgui.Button(u8"Реконнект "..fa.ICON_REFRESH, imgui.ImVec2(133, 35)) then
	...
	if vkladki[4] == true then
		...
		--[[	imgui.PushItemWidth(100) if imgui.InputInt('##key', key, 0) then
			 ...
			 imgui.SameLine()
			ShowHelpMarker(u8'Узнать ид клавиш, можно тут:  https://blast.hk/threads/8760/')]]--
			imgui.NewLine()
