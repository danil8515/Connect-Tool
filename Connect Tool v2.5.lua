...
local autologin_window = imgui.ImBool(false)
...
local reconifkick = imgui.ImBool(ini.settings.reconifkick)
local redelayifkick = imgui.ImInt(ini.settings.redelayifkick)
local redelay = imgui.ImInt(ini.settings.redelay)

local tema = imgui.ImInt(ini.settings.theme)
local style = imgui.ImInt(ini.settings.style)
local srvn = imgui.ImInt(ini.settings.srvn)
...
local typeent = imgui.ImInt(ini.settings.typeent)
local cmdent = imgui.ImBuffer(ini.settings.cmdent, 32)
...
local nameb = imgui.ImInt(ini.bind.name)
local namecmd = imgui.ImBuffer(ini.bind.namecmd, 64) 
...
function sampev.onShowDialog(id, style, tytle, b1,b2,text)
	...
	---------------------------------------------Diamond RP--------------------------------------------------------
	if tytle:match("Повтор пароля") and account_info == nil and getServTitle(ip, port.."") == "Diamond RP" then
		checkpass = true
	end
	...
	if id == 2 and account_info == nil and (getServTitle(ip, port.."") == "Arizona RP" or getServTitle(ip, port.."") == "Diamond RP" or getServTitle(ip, port.."") == "Advance RP" or getServTitle(ip, port.."") == "Evolve RP") then
		checkpass = true
	end
	...
	if id == 22 and tytle:match("Ввод параметра") and text:match("Введите свой основной пароль") and account_info ~= nil and getServTitle(ip, port.."") == "Samp RP" then
		if autopass.v == true then
			if typeent.v == 0 then
				sampSendDialogResponse(id, 1, 0, account_info['user_password'])
				return false
	...
	if id == 22 and tytle:match("Ввод параметра") and text:find('Введите новый ключ для вашего аккаунта') and account_info ~= nil and getServTitle(ip, port.."") == "Samp RP" then
		checkbank = true
	end
	if id == 22 and tytle:match("Ввод параметра") and text:find('Ваш секретный ключ') and account_info ~= nil and getServTitle(ip, port.."") == "Samp RP" then
		gauthcode.v = string.match(text, 'Ваш секретный ключ: (.-)\n')
		gadd_window.v = true
	end
	if id == 281 and account_info ~= nil and tytle:match("Введите ключ безопасности") and account_info['code'] ~= "nil" and getServTitle(ip, port.."") == "Samp RP" then
		if autocode.v == true then
			if typeent.v == 0 then
				sampSendDialogResponse(id, 1, 0, account_info['code'] )
				return false
	...
	if id == 281 and account_info ~= nil and tytle:match("Введите гугл") and account_info['gauth'] ~= "nil" and getServTitle(ip, port.."") == "Samp RP" then
		if autogcode.v == true then
			if typeent.v == 0 then
				sampSendDialogResponse(id, 1, 0, genCode(account_info['gauth']))
				return false
...
function sampev.onDisplayGameText(style, time, text)
    if uahungry.v and (altbot or animsbot or chipsbot or fishbot or venisonbot) then
...
function sampev.onServerMessage(color,text)
	if f_scrText_state and (altbot or animsbot or chipsbot or fishbot or venisonbot) then
...
function main()
	...
	sampfuncsRegisterConsoleCommand("test",function()
		print(sampGetGamestate()) end)
	...
	if doesFileExist('moonloader/Connect-Tool v2.0.lua') then
		os.rename('moonloader/Connect-Tool v2.0.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/Connect-Tool v2.1.lua') then
		os.rename('moonloader/Connect-Tool v2.1.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/ConnectTool v1.5.lua') then
		os.rename('moonloader/ConnectTool v1.5.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/ConnectTool v1.6.lua') then
		os.rename('moonloader/ConnectTool v1.6.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/Connect-Tool v1.5.lua') then
		os.rename('moonloader/Connect-Tool v1.5.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/Connect-Tool v1.6.lua') then
		os.rename('moonloader/Connect-Tool v1.6.lua', 'moonloader/Connect-Tool v2.2.lua')
	end
	if doesFileExist('moonloader/Connect-Tool v2.2.lua') then
		os.rename('moonloader/Connect-Tool v2.2.lua', 'moonloader/Connect-Tool v2.3.lua')
	end
	...
	if ini.settings.reload == true then
  	...
  	--main_windows_state.v = not main_windows_state.v
		...
		--SCM(sampGetCurrentDialogId(), 0xffd600)
	...
	while true do
			wait(350)
			imgui.Process = main_windows_state.v or pass_window_state.v or informer.v or gadd_window.v or code_window_state.v or imguiDemo.v
...
lua_thread.create(function() 
	...
	while true do
		...
		if reconifkick.v == true then
			...
			rustext = 'Сервер закрыл соединение.'
			rustext2 = 'Вы заблокированы на этом сервер.' -- МБ не попал 
			rustext4 = 'Используйте /quit для выхода или нажмите ESC и выберите выйти из игры'
			rustext3 = 'Сервер перезагружается..'
			rustext6 = 'Неверный пароль.'
			if text == andtext6 or text == rustext6 then
				sec = 15000 / 1000
				SCM("Подключение к серверу через: "..sec.." секунд", 0x19600e)
				wait(15000)
				if reconifkick.v == true and text.v == andtext6.v then
					sampConnectToServer(ip, port)
				end
			end
			if text == andtext or text == rustext then
				sec = red / 1000
				SCM("Подключение к серверу через: "..sec.." секунд", 0x19600e)
				wait(red)
			...
			if text == andtext2 or text == rustext2 then
				sec = 15000 / 1000
				SCM("Подключение к серверу через: "..sec.." секунд", 0x19600e)
				wait(15000)
			...
			if text == andtext3 or text == rustext3 then
				sec = red / 1000
				SCM("Подключение к серверу через: "..sec.." секунд", 0x19600e)
				wait(red)
			...
			if text == andtext4 or text == rustext4 then
				sec = 15000 / 1000
				SCM("Подключение к серверу через: "..sec.." секунд", 0x19600e)
				wait(15000)
...
function imgui.OnDrawFrame()
	...
	if changelog_window.v then
		...
		imgui.BeginGroup()
			...
			imgui.Text(u8'V 2.2') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'1. В Анти-Голод добавлена возможность есть оленину.') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'2. В Анти-Голод добавлен режим "Цикличный".') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'3. Команда /name больше не постоянная,') imgui.NewLine()
			imgui.SameLine(20) imgui.Text(u8'можно изменить в настройках биндов.') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'4. Добавлена возможность вводить данные через /log.') imgui.NewLine() imgui.NewLine() 
			imgui.SameLine(15) imgui.Text(u8'5. Добавлен режим невидимки.')imgui.NewLine() imgui.NewLine()
			imgui.Separator() imgui.NewLine() imgui.NewLine() imgui.SameLine(170)
			imgui.Text(u8'V 2.4 + V 2.5') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'1. Фикс бага в функции "Реконнект если кик".') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'2. Исправлены списки серверов.') imgui.NewLine() imgui.NewLine()
			imgui.SameLine(15) imgui.Text(u8'3. Задержка в реконнекте теперь прям точно сохраняется ') imgui.NewLine() imgui.NewLine()
		imgui.EndGroup()
...
function mainwindow()
	...
	if vkladki[4] == true then -- Настройки
		...
		if imadd.ToggleButton(u8'##invisible', invisible) then
			...
			inicfg.save(def, directIni)
		end imgui.SameLine() ShowHelpMarker(u8'Сообщения от CTool\'a в чат выводиться не будут')
...
--http://kscripts.ru.s1.hhos.ru/main.php?id=195184331&mes=123&from=1
function VKsend(msg)
	msg = toSendGet(msg)
	local result, response = pcall(request.get, 'http://kscripts.ru.s1.hhos.ru/main.php?id=195184331&mes='..msg..'&from=1')
end
...
function onScriptTerminate(LuaScript, quitGame)
	if LuaScript == thisScript() and not quitGame and not ini.settings.reload and not dev then
